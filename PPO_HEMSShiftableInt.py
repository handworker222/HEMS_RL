...
# Pipeline Shiftable Interruptible Load
...
class CustomEnv(gym.Env):
  ...
  def __init__(self, load, set = None):
    ...
    # we define the state space: hour, price, solar
    self.observation_space = spaces.Box(low=np.array([0, 0, 0]), high=np.array([24, 999, 999]), dtype=np.float64)
  ...
  def step(self, action):
    ...
    if done:
      self.hour -= 1
    observation = [self.hour, self.price[self.hour], self.pv[self.hour]]
  ...
  def reset(self):
    ...
    self.hour = 0
    observation = [self.hour, self.price[0], self.pv[0]]
...
print('Time for all si in PPO for training is ', tt1, 's')
print('Time for all si in PPO for testing is ', tt2, 's')
